...
namespace Izh_04_Basic_Coding
{
    ...
    public class Izh04BasicCodingTests
    {
        ...
        {
            return MaxFinder.FindMax(sourceArray);
        }

        [TestCase(null,ExpectedResult = typeof(NullReferenceException))]
        [TestCase(new int[0],ExpectedResult = typeof(ArgumentException))]
        public Type CheckFindMaxExceptions(int[] sourceArray)
        {
            try
            {
                MaxFinder.FindMax(sourceArray);
            }
            catch (Exception ex)
            {
                return ex.GetType();
        ...
        [TestCase(new double[] { 0, 1, 2, 3 }, ExpectedResult = null)]
        ...
        [TestCase(new double[] { 0, 0},ExpectedResult = null)]
        public int? CheckFindingElementIndexBetweenEqualSums(double[] sourceArray)
        {
            return IBESFinder.FindIEBS(sourceArray);
        }

        [TestCase(null, ExpectedResult = typeof(NullReferenceException))]
        public Type IBESExceptionsTest(double[] sourceArray)
        {
            try
            {
                IBESFinder.FindIEBS(sourceArray);
            }
            catch (Exception ex)
            {
                return ex.GetType();
        ...
        [TestCase(321, ExpectedResult = null)]
        [TestCase(21, ExpectedResult = null)]
        [TestCase(3456432, ExpectedResult = 3462345)]
        public int? CheckFindNextBigger(int sourceValue)
        {
            return NextBiggerFinder.FindNextBigger(sourceValue);
        }

        [TestCase(876593248, ExpectedResult = true)]
        public bool CheckFindNextBiggerWorkTime(int sourceValue)
        {
            var time = 0L;
            NextBiggerFinder.FindNextBigger(sourceValue, out time);
            return time < 10L;
        }
        ...
        [TestCase(new int[] { 7, 1, 2, 3, 4, 5, 6, 7, 68, -69, 70, 15, 17 }, 9, ExpectedResult = new int[] { -69 })] // в тесте был пустой массив. Исправлено на -69, но мб я что-то не так понял
        public int[] CheckDigitFiltering(int[] sourceArray, int digit)
        {
            return DigitFilter.FilterByDigitContainment(sourceArray, (byte)digit);
        }
    }
}
